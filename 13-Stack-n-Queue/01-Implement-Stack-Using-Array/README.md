...
- [Striver's SDE Sheet](https://takeuforward.org/data-structure/implement-stack-using-array)

---

## Implementation using vectors (dynamic array)
...
class myStack {
private:
    std::vector<int> vec; // The vector replacing the raw array
    int maxSize;          // Still need this to enforce the stack's capacity limit

public:
    myStack(int n) {
        maxSize = n;
        // reserve() allocates the memory upfront for efficiency,
        // but keeps the current size at 0 until we push elements.
        vec.reserve(maxSize);
    }

    bool isEmpty() {
        // Vectors have a built-in method to check if they are empty
        return vec.empty();
    }

    bool isFull() {
        // The stack is full if the vector's current size reaches our max capacity
        return vec.size() == maxSize;
    }

    void push(int x) {
        if(isFull()) return;
        // push_back automatically adds the element to the "top" (end) of the vector
        vec.push_back(x);
    }

    void pop() {
        if(isEmpty()) return;
        // pop_back automatically removes the element from the "top" (end)
        vec.pop_back();
    }

    int peek() {
        if(isEmpty()) return -1; // Prevent crashing on empty stack
        // back() returns the last element in the vector (our stack's top)
        return vec.back();
    }
};
```
